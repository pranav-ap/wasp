...
void Compiler::visit(const Statement_ptr statement) {
  ...
  std::visit(overloaded{[&](ExpressionStatement &stat) { visit(stat); },
...
void Compiler::compile_variable_definition(const Expression_ptr &assignment,
                                           bool as_expression) {
...
void Compiler::visit(ForInLoop &statement) {
  ...
  loop_tracking_stack.emplace_back(header, body, end,
                                   current_lexical_scope_depth,
                                   current_lexical_scope_depth);
...
void Compiler::visit(LoopControl &statement) {
  ...
  auto [header, body, end, entry_depth, body_depth] =
      loop_tracking_stack.back();
  ...
  ASSERT(scopes_to_pop >= 0, "Compiler Error: Current lexical scope depth is "
...
void Compiler::visit(FunctionDefinition &statement) {
  ...
  int const_id =
      constant_pool->allocate_function_definition(std::move(func_code));
...
void Compiler::visit(const Expression_ptr expr) {
  ...
  std::visit(
      overloaded{
          [&](int val) { visit(val); }, [&](double val) { visit(val); },
          [&](std::string val) { visit(val); }, [&](bool val) { visit(val); },
          [&](Identifier &id) { visit(id); },
          [&](UntypedAssignment &a) { visit(a); },
          [&](TypedAssignment &a) { visit(a); }, [&](Prefix &p) { visit(p); },
          [&](Infix &i) { visit(i); }, [&](Call &c) { visit(c); },
          [&](ListLiteral &l) { visit(l); }, [&](TupleLiteral &t) { visit(t); },
          [&](MapLiteral &m) { visit(m); }, [&](SetLiteral &s) { visit(s); },
...
void Compiler::visit(int expr) {
  emit(OpCode::LOAD_CONST, constant_pool->allocate(expr));
}
...
void Compiler::resolve_jumps_in_block(
    ByteVector &bytes, const std::map<BlockId, size_t> &offsets) const {
  size_t ip = 0;
  while (ip < bytes.size()) {
    ...
    if (op == OpCode::JUMP || op == OpCode::JUMP_IF_FALSE ||
        op == OpCode::LOOP_ITER) {
      BlockId target =
          static_cast<BlockId>(static_cast<uint8_t>(bytes[ip + 1]) |
...
CodeObject Compiler::flatten() {
  ...
  for (const auto &block : all_blocks) {
    ...
    if (block_code.length() > 0) {
      ByteVector temp_bytes(block_code.data(),
                            block_code.data() + block_code.length());
