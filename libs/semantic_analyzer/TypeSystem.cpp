...
bool TypeSystem::equal(
    ...
    if (type_1->is<VariantType>() && type_2->is<VariantType>()) {
        return equal_unordered(
            scope, type_1->as<VariantType>().types, type_2->as<VariantType>().types
        );
    }
...
bool TypeSystem::equal_unordered(
    ...
    for (auto left : left_vector) {
        bool found = std::any_of(right_vector.begin(), right_vector.end(), [&](auto right) {
...
bool TypeSystem::assignable(
    ...
    if (rhs_type->is<VariantType>()) {
        ...
        return std::all_of(rhs_var.types.begin(), rhs_var.types.end(), [&](Object_ptr t) {
    ...
    if (lhs_type->is<VariantType>()) {
        ...
        return std::any_of(lhs_var.types.begin(), lhs_var.types.end(), [&](Object_ptr t) {
