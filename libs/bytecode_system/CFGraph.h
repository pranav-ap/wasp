...
public:
    ...
    CodeObject(ByteVector instrs, StringVector comments)
        : instructions(std::move(instrs)), comments(std::move(comments))
    {
    }
    ...
    std::string get_comment_at(std::size_t index) const;
...
public:
    ...
    const CodeObject& get_code() const
    {
        return code;
    }

    const std::vector<BlockId>& get_successors() const
    {
        return successors;
    }
    const std::vector<BlockId>& get_predecessors() const
    {
        return predecessors;
    }
};
...
public:
    ...
    const std::vector<BasicBlock>& get_all_blocks() const
    {
        return blocks;
    }
};
