...
#define OPCODE_LIST(X)                                                              \
    ...
    /* --- Constants & Memory --- */                                                \
    X(LOAD_CONST,                                                                   \
      1)            /* [] -> [val] | <Constant Pool ID> (index of literal value) */ \
    X(LOAD_TRUE, 0) /* [] -> [true] */                                              \
    ...
    X(POP_SCOPE_KEEP_TOS,                                                           \
      0) /* Exit current lexical block scope, preserving Top Of Stack */            \
    ...
    X(SET_UPVALUE, 1) /* [val] -> [] | <Upvalue Index> */                           \
                                                                                    \
    /* --- Classes --- */                                                           \
    X(BUILD_OVERLOAD_GROUP,                                                         \
      1) /* [closures...] -> [overload group] | <number of overloads> */            \
    X(PUSH_EMPTY_OVERLOAD_GROUP, 0)                                                 \
    X(BUILD_CLASS, 2) /* [overload groups...] -> [class blueprint] | <methods       \
                         count>, <fields count> */                                  \
    X(PUSH_EMPTY_CLASS_BLUEPRINT, 0)                                                \
    X(INSTANTIATE,                                                                  \
      1) /* [class blueprint, args...] -> [instance] | <number of args> */          \
                                                                                    \
    X(BUILD_TEMPLATE, 1)                                                            \
    /* --- Arithmetic --- */                                                        \
    X(NEGATE, 0) /* [a] -> [-a] */                                                  \
    ...
    X(BUILD_MAP,                                                                    \
      1) /* [k1, v1...] -> [map] | <Count> (Number of key-value pairs) */           \
    X(BUILD_SET, 1) /* [v1...vn] -> [set] | <Count> (Number of items to pop) */     \
    X(BUILD_RANGE,                                                                  \
      1) /* [start, stop, step] -> [range] | <Count> (Arguments provided) */        \
                                                                                    \
