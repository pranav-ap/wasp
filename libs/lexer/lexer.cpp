...
std::vector<Token> Lexer::run(std::string_view source_code) {
    source = source_code;
...
Token Lexer::consume_operators() {
    ...
    switch (current_char) {
        case '+': return consume_plus();
        case '-': return consume_minus();
        case '*': return consume_star();
        case '/': return consume_division();
        case '%': return consume_reminder();
        case '^': return consume_power();
        case '!': return consume_bang();
        case '=': return consume_equal();
        case ':': return consume_colon();
        case '.': return consume_dot();
        case '?': return consume_question();
        case '>': return consume_greater_than();
        case '<': return consume_lesser_than();
        default: return consume_single_char_punctuation(current_char);
    }
}
...
Token Lexer::consume_dot() {
    ...
    if (expect_current_char('.')) {
        if (expect_current_char('.')) {
            return Token(TokenType::DOT_DOT_DOT, "...", LINE_NUM, COL_NUM);
        }
...
Token Lexer::consume_single_char_punctuation(char ch) {
    switch (ch) {
        case '(': next(); return Token(TokenType::OPEN_PARENTHESIS, "(", LINE_NUM, COL_NUM);
        case ')': next(); return Token(TokenType::CLOSE_PARENTHESIS, ")", LINE_NUM, COL_NUM);
        case '{': next(); return Token(TokenType::OPEN_CURLY_BRACE, "{", LINE_NUM, COL_NUM);
        case '}': next(); return Token(TokenType::CLOSE_CURLY_BRACE, "}", LINE_NUM, COL_NUM);
        case '[': next(); return Token(TokenType::OPEN_SQUARE_BRACKET, "[", LINE_NUM, COL_NUM);
        case ']': next(); return Token(TokenType::CLOSE_SQUARE_BRACKET, "]", LINE_NUM, COL_NUM);
        case '\\': next(); return Token(TokenType::BACKWARD_SLASH, "\\", LINE_NUM, COL_NUM);
        case ',': next(); return Token(TokenType::COMMA, ",", LINE_NUM, COL_NUM);
        case '|': next(); return Token(TokenType::VERTICAL_BAR, "|", LINE_NUM, COL_NUM);
        case '_': next(); return Token(TokenType::UNDERSCORE, "_", LINE_NUM, COL_NUM);
        case '$': next(); return Token(TokenType::DOLLAR, "$", LINE_NUM, COL_NUM);
        case '~': next(); return Token(TokenType::TILDE, "~", LINE_NUM, COL_NUM);
        case '@': next(); return Token(TokenType::AT_SIGN, "@", LINE_NUM, COL_NUM);
        case '#': {
            next(); 
        ...
        default: return consume_unknown_token();
    }
}
...
{
	int index = source_code_pointer.get_index();
