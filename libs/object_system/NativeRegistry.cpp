...
void NativeRegistry::add_native(const std::string &name, int arity,
                                NativeFnType function, ObjectVector input_types,
  ...
  auto obj =
      MAKE_SHARED_OBJECT_VARIANT(NativeFunctionObject, function, arity, name);
  ...
  add_native(
      ...
      [&](const std::vector<Object_ptr> &args) -> Object_ptr {
        for (const auto &arg : args) {
          std::visit(
              overloaded{[](const IntObject &i) { std::cout << i.value; },
