...
void Parser::register_all_parselets()
{
    ...
    register_parselet(TokenType::DOT, std::make_shared<MemberAccessParselet>());
}

void Parser::register_parselet(
    TokenType token_type,
    IPrefixParselet_ptr parselet
)
{
...
{
    prefix_parselets.insert(make_pair(
        token_type,
        std::make_shared<PrefixOperatorParselet>(static_cast<int>(precedence))
    ));
}
...
void Parser::register_infix_left(TokenType token_type, Precedence precedence)
{
    register_parselet(
        token_type,
        std::make_shared<InfixOperatorParselet>(
