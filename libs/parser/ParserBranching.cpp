...
namespace Wasp
{
	...
	Statement_ptr Parser::parse_branching(TokenType token_type, int if_keyword_indent_level)
	{
		...
		if (token_pipe.peek_type_at_indent(if_keyword_indent_level, TokenType::ELIF))
		{
			token_pipe.expect_n_indents(if_keyword_indent_level);
			auto alternative = parse_branching(TokenType::ELIF, if_keyword_indent_level);
			return MAKE_STATEMENT(IfBranch(condition, body, alternative));
		}

		if (token_pipe.peek_type_at_indent(if_keyword_indent_level, TokenType::ELSE))
		{
