...
Token TokenPipe::require(TokenType token_type) {
  if (const auto token = current()) {
    ...
    cout << "Error: Expected token of type " << to_string(token_type)
         << " but got " << to_string(token->type) << " at line " << token->line
         << endl;
...
Token TokenPipe::require(const std::vector<TokenType> &token_types) {
  ...
  if (token) {
    ...
    for (size_t i = 0; i < token_types.size(); ++i) {
      std::cout << to_string(token_types[i])
                << (i < token_types.size() - 1 ? ", " : " ");
    }

    std::cout << "} but got " << to_string(token->type) << " at line "
              << token->line << std::endl;
    exit(1);
  }

  std::cout << "Error: Expected one of multiple types but got end of file"
            << std::endl;
  exit(1);
}
...
void TokenPipe::expect_n_indents(const int n) {
  for (int i = 0; i < n; i++) {
    if (const auto token = current(); token->type != TokenType::TAB) {
      cout << "Error: Expected " << n << " indents but got " << i << " at line "
           << token->line << endl;
      exit(1);
    }
